...
def save_desc(basename, descs, kind='desc'):
    with open(basename + ".%s.txt" % kind, 'w', encoding="utf-8") as outf:
...
def load_desc(basename, kind='desc'):
    ...
    if os.path.exists(descname):
        with open(descname, encoding="utf-8") as descf:
            for line in descf.read().split('\n'):
                ...
                if len(parts) < 2:
                    logger.warning("No tags on line '%s' of file %s", line, descname)
                    continue
                (item_id, desc) = (parts[0], parts[1])
