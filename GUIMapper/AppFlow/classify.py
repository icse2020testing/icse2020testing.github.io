...
def preprocess_img(filename):
    ...
    if os.path.exists(ocrfile):
        with open(ocrfile, encoding="utf-8") as ocrf:
            ocrret = ocrf.read()
    else:
...
def evaluate(datapts, apps, tags, app=None, evalscreen=None):
    ...
    predictions = clas.predict(test_keys)
    for idx in range(len(test_files)):
        line_list = []
        line_list.append(app)
        line_list.append(test_files[idx])
        line_list.append(predictions[idx])
        line_list.append(test_lbls[idx])
        with open('prediction_screen_results.csv', 'a') as csv_file:
