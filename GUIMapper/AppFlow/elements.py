...
def evaluate(scr_points, scr_labels, scr_apps, indices):
    ...
    for idx in range(len(X_test)):
        ...
        correct = y_test[idx]
        #   add relevant properties of point to list
        point_list = []
        point = X_test[idx]
        point_list.append(point['app'])
        file_name = point['app'] + '_' + point['case'] + '_' + point['scr']
        point_list.append(file_name)
        point_list.append(point['id'])
        point_list.append(pred)
        point_list.append(correct)

        with open('prediction_results.csv', 'a') as csv_file:
